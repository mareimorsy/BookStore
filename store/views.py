...
def index(request):
    if request.user.is_authenticated:
        MostReadBooks = Book.objects.raw('select store_book_read.book_id,store_book.id,store_book.title, count(store_book_read.book_id) as books  from store_book_read, store_book where store_book.id = store_book_read.book_id group by book_id order by books desc limit 2')
        topRatedBooks = Book.objects.raw('select store_rate.book_id,store_book.id, title, avg(rate) as rate from store_rate, store_book where store_rate.book_id = store_book.id group by book_id order by rate desc limit 5')
        topFollowedAuthors = Author.objects.raw('select store_author_follow.author_id,store_author.id, count(store_author_follow.author_id) as followers from store_author_follow, store_author where store_author.id = store_author_follow.author_id group by author_id order by followers desc limit 5')
...
def getBook(request, book_id):
    if request.user.is_authenticated:
        ...
        categories = Category.objects.filter(book=book.id)
        if request.method == 'GET':
            show_read = False
            show_wish = False
            ...
            return render(request, 'book.html', {'book': book, 'show_read': show_read, 'show_wish': show_wish, 'rate_value':rate_value, 'categories':categories})
        else:
            # book_id
...
def search(request):
    books = Book.objects.filter( Q(title__icontains=request.GET.get('q')) | Q(author__name__icontains=request.GET.get('q')) )
    return render(request, 'books.html', {'books': books})
    # if request.user.is_authenticated:
    # else:
    #     return redirect('login')
...
def changePassword(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'your password has changeed successfully!')
